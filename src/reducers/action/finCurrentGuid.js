...
const TRANSACTION_ID_URL_CHARGE= '/api/get-transacctions-id-charge';
...
export const fetchQuoteDetailCharge = (guid) => {

    return (dispatch) => {
        dispatch(fetchQuoteDetailSuccess([]))
        axios
            .post(TRANSACTION_ID_URL_CHARGE, JSON.stringify({guid: guid}), {
                mode: 'no-cors',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + localStorage.getItem('accessToken'),
